...
- `SKW_TO_APEX_DDL.sql` — Full database schema: tables, views, PL/SQL packages
- `Wymagania.md` — Requirements document (Polish)
- `program/readable/application/` — APEX application export (YAML format, app ID 160, alias START338)
...
1. Execute `SKW_TO_APEX_DDL.sql` against the Oracle database (schema `DAW`)
2. Import the `program/` directory into Oracle APEX using the APEX import tool or `apex export`/`apex import` CLI
